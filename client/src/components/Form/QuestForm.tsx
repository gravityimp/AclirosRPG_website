...
const QuestForm: FC<Props> = (props) => {
        ...
        if (editQuest) {
            if (editQuest.requiredQuests) {
                ...
                editQuest.requiredQuests.forEach((q) => {
                    const _quest = allQuests.find((_q) => _q.id === q);
                    if (_quest) {
                        _quests.push(_quest);
                    }
                });
    ...
    return (
        <Box sx={styles.box}>
            <Box sx={styles.formBox}>
                <form style={{ overflowY: "auto", width: "100%" }}>
                    ...
                    <Box sx={{ display: 'flex', flexDirection: 'column', justifyContent: 'center', alignItems: 'center' }}>
                        <Typography variant="h2" sx={{ marginBottom: '8px' }}>Stages</Typography>
                        <Stack
                            sx={{ width: '100%' }}
                            spacing="4px"
                        >
                            {
                                stages.map((stage, index) => {
                                    return (
                                        <Stack key={stage.stageName} direction="row" sx={{ display: "flex", alignItems: "center", padding: '8px', borderRadius: '4px', width: "90%", justifyContent: 'flex-start', bgcolor: 'orange' }}>
                                            <Typography sx={{ width: "100%" }}>NAME: {stage.stageName}<Chip color="success" label={stage.stageType} /></Typography>
                                            <IconButton
                                                color="primary"
                    ...
                    <Box>
                        ...
                        <Stack spacing="4px">
                            {
                                rewards.items && rewards.items.length > 0 && rewards.items.map((item, index) => {
                                    return (
                                        <Box key={item.itemId} sx={{ display: 'flex', flexDirection: 'row', justifyContent: 'center', alignItems: 'center', width: '100%', marginY: '8px' }}>
                                            ...
                                            <TextField
                                                ...
                                                InputProps={{
                                                    inputProps: {
                                                        min: 1,
                                                        max: minecraftItems.find(mi => mi.displayName.toLocaleLowerCase() === allItems.find(ai => ai.id === item.itemId)?.name?.toLocaleLowerCase()) || 1,
                                                    }
                                                }}
                                            />
                                            <IconButton
                            ...
                            <Box sx={{ display: 'flex', flexDirection: 'row', justifyContent: 'center', alignItems: 'center', width: '100%', marginY: '4px' }}>
                                ...
                                <TextField
                                    ...
                                    InputProps={{
                                        inputProps: {
                                            min: 1,
                                            max: minecraftItems.find(mi => mi.displayName.toLocaleLowerCase() === addRewardItem.name.toLocaleLowerCase())?.stackSize || 1
                                        }
                                    }}
                                />
                                <IconButton
            ...
            <EditModal isOpen={stageModal} onClose={() => setStageModal(false)}>
                <StageForm
